...
class ClipDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        all_clips = torchvision.transforms.RandomHorizontalFlip(p=0.5)(all_clips)
        if self.train_val == "train":
            if np.random.randn(1) > 0.5:
                all_clips = self.tf1(all_clips)
            else:
                all_clips = self.tf2(all_clips)
        lip_portion = all_clips[
            :5, :, 224 * self.lip_upscale : 480 * self.lip_upscale, 128 * self.lip_upscale : 384 * self.lip_upscale
        ]
        if lip_portion.shape[2] != (self.lip_upscale) * 256:
            raise ValueError(
                f"Lip portion size {lip_portion.shape[2]} does not match expected size {self.lip_upscale * 256}"
            )
        if all_clips.shape[2] != self.upscale * 256:
            all_clips = torchvision.transforms.functional.resize(all_clips, (256 * self.upscale, 256 * self.upscale))
        all_clips = all_clips / 255.0
        lip_portion = lip_portion / 255.0
...
if __name__ == "__main__":
    df = ClipDataset(
        ...
        json_loc="/data/prakash_lipsync/v1/hdtf/train_hallo.json",
        augment_num=1,
        upscale=2,
        lip_upscale=1,
        train_val="train",
    ...
    x3 = x[1]
    x3 = torchvision.transforms.ToPILImage()(torchvision.utils.make_grid(x3, nrow=5))
    x3.save("temp/lip_portion.png")
