...
class ClipDataset(Dataset):
    def __init__(
        ...
        self.length = length
        self.landmarks_folder = Path(landmarks_folder)
        self.silence_encoding = load_file(silence_encoding)["audio_embedding"]
    ...
    def resize(self, img, size=512):
        size = self.image_size or size
        return torchvision.transforms.Resize(size)(img)
    ...
    def remove_bad_clips(self, records):
        ...
        for video_name, video_data in records_copy.items():
            ...
            if all(clip["silence_filtered"] for clip in clips):
    ...
    def __getitem__(self, _):
        ...
        try:
            ...
            pick_silence = np.random.rand() < 0.2
            if pick_silence:
                shut_mouth_frames = []
                for i in range(len(data)):
                    for ld, img in zip(data[i]["lip_distances"], data[i]["images"]):
                        if ld < 0.1:
                            shut_mouth_frames.append((i, img))
                            break
                if len(shut_mouth_frames) == 0:
                    return self[0]
                bin_no, img = random.sample(shut_mouth_frames, 1)[0]
                source_loc = (
                    self.data_root / self.video_folder / self.ds_name / video_name / data[bin_no]["folder_name"] / img
                )
                # prev_bin_no = max(0, bin_no - 1)
                source_image_data = torchvision.io.read_image(source_loc, mode=torchvision.io.ImageReadMode.RGB)
                source_image_data = self.resize(source_image_data)
                source_clip_list = [source_image_data] * 5
                sil_start = random.randint(0, len(self.silence_encoding) - 5)
                sil_end = sil_start + 5
                silence_anchor = 1
                deep_speech_clip = self.silence_encoding[sil_start:sil_end]
                reference_clip_list = []
                # get 5 reference images from different bins
                available_indices = list(range(len(data)))
                available_indices.remove(bin_no)
                reference_anchor_list = random.sample(available_indices, 5)

                lip_distances = []
                reference_frame_loc_list = []
                for _, reference_anchor in enumerate(reference_anchor_list):
                    ...
                    reference_image_data = torchvision.io.read_image(
                        reference_loc, mode=torchvision.io.ImageReadMode.RGB
                    )
                    reference_image_data = self.resize(reference_image_data)
                    reference_clip_list.append(reference_image_data)
                reference_clip_list = torch.stack(reference_clip_list)
                reference_clip_list = [reference_clip_list] * 5
                deep_speech_list = [deep_speech_clip] * 5
                deep_speech_syncnet = self.silence_encoding[50:59]
                silence_anchor = 1
                ...
                start_frame = 0
                # prev_bin_no = max(0, source_anchor - 1)
                for source_frame_index in range(start_frame, 5):
                    ...
                    source_audio_data = wvf[
                        source_image_path_list["start_audio"]
                        + source_frame_index
                        + start_audio : source_image_path_list["start_audio"] + source_frame_index + end_audio,
                        ...,
                    ]
                    ...
                    if silence_anchor == 1:
                        available_indices = [i for i in available_indices if not data[i]["silence_filtered"]]
                    ...
                    for _, reference_anchor in enumerate(reference_anchor_list):
                        reference_frame_path_list = data[reference_anchor]
                        reference_frame_index = random.sample(range(len(reference_frame_path_list["images"])), 1)[0]
                        lip_distances.append(data[reference_anchor]["lip_distances"][reference_frame_index])
                        # frame
                        reference_loc = (
                            self.data_root
                            / self.video_folder
                        ...
                        reference_frame_loc_list.append(reference_loc)
                    for i, reference_loc in enumerate(reference_frame_loc_list):
                        reference_image_data = torchvision.io.read_image(
                            str(source_loc) if (i == 0) and (self.train_val != "train") else str(reference_loc),
                            mode=torchvision.io.ImageReadMode.RGB,
                        )
                        reference_image_data = self.resize(reference_image_data)
                        if self.reference_frames_transforms:
                            if self.train_val == "train":
                                reference_image_data = RandomPerspective(distortion_scale=0.1, p=0.1)(
                                    reference_image_data
                                )
                                reference_image_data = RandomZoomIn(scale=(1, 1.2))(reference_image_data)
            ...
            # if all_clips.shape[2] != self.upscale * 512:
            #     raise ValueError(f"Image size {all_clips.shape[2]} does not match expected size {self.upscale * 512}")
            ...
            # reference_clip = F.resize(reference_clip, (512, 512))  # resize references to 256x256
            # reference_clip = torch.cat(torch.split(reference_clip, 5, dim=0), 1)
            reference_clip = rearrange(reference_clip, "(b f) c h w -> b (f c) h w", f=5)
            ...
            if not pick_silence:
                deep_speech_syncnet = wvf[
                    source_image_path_list["start_audio"] : source_image_path_list["end_audio"] + 1, ...
                ]
            # print(deep_speech_syncnet.shape)
            if self.silences:
                return (
                    source_clip,
                    reference_clip,
...
def load_concat_data(opt):
    num_train_datasets = len([d for d in opt.data if d.train_json is not None])
    ...
    train_data = [
        ClipDataset(
            ...
            length=1000 * opt.nodes * num_devices // num_train_datasets,  # make sure each epoch is 1000 steps
            landmarks_folder=Path(d.landmarks_folder),
            silence_encoding=Path(d.silence_encoding),
    ...
    val_data = [
        ClipDataset(
            ...
            reference_frames_transforms=d.reference_frames_transforms,
            length=50 * opt.nodes * num_devices // num_val_datasets,  # make sure each epoch is 50 steps
            landmarks_folder=Path(d.landmarks_folder),
            silence_encoding=Path(d.silence_encoding),
            image_size=opt.image_size,
        )
...
if __name__ == "__main__":
    df = ClipDataset(
        "/data/wider_crop_data_512",
        ds_name="iv_recording",
        audio_features_name="fe_wav2vec12",
        video_folder="video",
        json_loc="/data/wider_crop_data_512/jsons/iv_recording/val_silence_filtered_new.json",
        augment_num=4,
        upscale=1,
        train_val="train",
