...
from lipsync.latentsync_ds.affine_transform import AlignRestore, laplacianSmooth
...
def load_fixed_mask(resolution: int) -> torch.Tensor:
    mask_image = cv2.imread("/home/prakash/lipsync/lipsync/latentsync_ds/mask.png")
...
if __name__ == "__main__":
    image_processor = ImageProcessor(512, mask="fix_mask", device="cuda")
    # videos = fc.L(fc.Path("/data/lipsync_raw_data/iv_recording_v2/").glob("*.mp4"))
    # video = cv2.VideoCapture(str(videos[np.random.randint(0, len(videos))]))
    video = cv2.VideoCapture("nbs/videos/video_001.mp4")
    mask = cv2.imread("/home/prakash/lipsync/lipsync/latentsync_ds/mask.png")
    ...
    while True:
        ...
        # frame = torch.Tensor(frame).type(torch.uint8)
        ...
        try:
            ...
            face_masked = np.where(mask == 0, 255, face)
            # concat face, mask, face_masked
            face_masked = np.concatenate([face, mask, face_masked], axis=1)
            cv2.imwrite(f"temp/latent_sync/face_{n}.jpg", face_masked)
