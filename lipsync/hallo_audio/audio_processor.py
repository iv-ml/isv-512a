...
class AudioProcessor:
    ...
    def preprocess(self, wav_file: str, clip_length: int = -1):
        ...
        if self.audio_separator is not None:
            ...
            vocal_audio_file = os.path.join(
                self.audio_separator.output_dir, vocal_audio_file
            )
            vocal_audio_file = resample_audio(
                vocal_audio_file,
                os.path.join(
                    self.audio_separator.output_dir, f"{vocal_audio_name}-16k.wav"
                ),
                self.sample_rate,
            )
        else:
        ...
        speech_array, sampling_rate = librosa.load(
            vocal_audio_file, sr=self.sample_rate
        )
        ...
        if clip_length > 0 and seq_len % clip_length != 0:
            audio_feature = torch.nn.functional.pad(
                ...
                (
                    0,
                    (clip_length - seq_len % clip_length)
