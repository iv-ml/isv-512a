...
from lipsync.dinet.model_adv import DINetSPADE2
...
class DINetClipLightningModule(L.LightningModule):
    def __init__(self, opt):
        ...
        self.opt = opt
        self.net_g = DINetSPADE2(
            opt.source_channel,
            opt.ref_channel,
            audio_seq_len=opt.audio_seq_len,
            upscale=opt.upscale,
            lip_upscale=opt.lip_upscale,
        ...
        self.net_dI_2 = EnhancedDiscriminator(
    ...
    def training_step(self, batch, batch_idx):
        ...
        # get a number between -2 to 2 using random
        if self.opt.random_mask_dim:
            x_rand = int(torch.randint(0, 8, (1,))) - 4
            # get a number between 0 to 8  using random
            y_rand = int(torch.randint(0, 16, (1,)))
            mask_dim = [mask_dim[0] + x_rand, mask_dim[1] + y_rand, mask_dim[2] + x_rand, mask_dim[3] + y_rand]
        opt_g, opt_d, opt_di_2, opt_lip_d = self.optimizers()
        (source_clip_v, lip_portion_v, reference_clip, deep_speech_clip, deep_speech_full, _) = batch

        gt_source_clip = torch.cat(torch.split(source_clip_v, 1, dim=1), 0).squeeze(1)
        gt_lip_portion = torch.cat(torch.split(lip_portion_v, 1, dim=1), 0).squeeze(1)
        ...
        # if self.opt.lip_upscale > 1:
        #     gt_lip_portion = F.interpolate(gt_lip_portion, scale_factor=1 / self.opt.lip_upscale, mode="bilinear")
        # else:
        #     gt_lip_portion = gt_lip_portion

        fake_out, fake_lip_out = self(source_clip, reference_clip, deep_speech_clip, mask_dim=mask_dim)
        ...
        self.toggle_optimizer(opt_di_2)
        opt_di_2.zero_grad()
        loss_d_face_2, _ = self.discriminator_2(fake_lip_out, gt_lip_portion)
        self.manual_backward(loss_d_face_2)

        if self.opt.clip_gradients:
            pre_clip_norm = check_gradient_magnitudes(self.net_dI_2)
            self.clip_gradients(self.net_dI_2, self.opt.disc_clip_value)
            post_clip_norm = check_gradient_magnitudes(self.net_dI_2)
            self.log(name="img_disc_2_post", value=post_clip_norm, batch_size=source_clip.shape[0], sync_dist=True)
            self.log(name="img_disc_2_pre", value=pre_clip_norm, batch_size=source_clip.shape[0], sync_dist=True)

        opt_di_2.step()
        self.untoggle_optimizer(opt_di_2)

        # lip discriminator
        self.toggle_optimizer(opt_lip_d)
        opt_lip_d.zero_grad()
        loss_d_lip, _ = self.lip_discriminator(fake_out, gt_source_clip, mask_dim=mask_dim)
        ...
        loss_d = (loss_d_face + loss_d_face_2 + loss_d_lip) / 3
        ...
        _, pred_fake_dI_2 = self.net_dI_2(fake_lip_out)
        _, pred_fake_lip_dI = self.lip_net_dI(
            fake_out[
                :,
                :,
                mask_dim[1] * self.opt.upscale : mask_dim[3] * self.opt.upscale,
                mask_dim[0] * self.opt.upscale : mask_dim[2] * self.opt.upscale,
            ]
        )
        ...
        loss["perception_loss_2"] = self.perception_loss(fake_lip_out, gt_lip_portion) * self.opt.lambda_perception
        loss["l1_loss_2"] = F.l1_loss(fake_lip_out, gt_lip_portion) * self.opt.lambda_perception * 5
        ...
        loss["gen_adv_loss_2"] = self.gan_loss(pred_fake_dI_2, True)
        loss["total_gen_loss"] = (
            loss["perception_loss"]
            + loss["l1_loss"]
            + loss["perception_loss_2"]
            + loss["l1_loss_2"]
            + loss["perception_lip_loss"]
            + loss["l1_lip_loss"]
            + loss["gen_adv_loss"]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # if self.opt.lip_upscale > 1:
        #     lip_portion = F.interpolate(gt_lip_portion, scale_factor=1 / self.opt.lip_upscale, mode="bilinear")
        # else:
        #     lip_portion = gt_lip_portion

        fake_out, fake_lip_out = self(source_clip, reference_clip, deep_speech_clip, mask_dim=[64, 112, 192, 240])
        fake_out_resized = F.interpolate(fake_out, scale_factor=1 / self.opt.upscale, mode="bilinear")
        _, pred_fake_dI = self.net_dI(fake_out)
        _, pred_fake_lip_dI = self.lip_net_dI(
            fake_out[
        ...
        loss["disc_loss_2"], _ = self.discriminator_2(fake_lip_out, gt_lip_portion)
        loss["lip_disc_loss"], _ = self.lip_discriminator(fake_out, gt_source_clip, mask_dim=[64, 112, 192, 240])
        loss["total_disc_loss"] = loss["disc_loss"] + loss["disc_loss_2"] + loss["lip_disc_loss"]
    ...
    def discriminator_2(self, fake_out, source_image_data):
        _, pred_fake_dI = self.net_dI_2(fake_out.detach())
        loss_dI_fake = self.gan_loss(pred_fake_dI, False)

        real_features, pred_real_dI = self.net_dI_2(source_image_data)
        loss_dI_real = self.gan_loss(pred_real_dI, True)
        loss_dI = (loss_dI_fake + loss_dI_real) * 0.5
        return loss_dI, real_features

    def lip_discriminator(self, fake_out, source_image_data, mask_dim):
    ...
    def configure_optimizers(self):
        ...
        opt_di_2 = optim.AdamW(self.net_dI_2.parameters(), lr=opt.lr_dI, fused=True)
        ...
        self.net_dI_2_scheduler = get_scheduler(opt_di_2, opt.non_decay, opt.decay, lr_policy=self.opt.scheduler)
        self.lip_net_dI_scheduler = get_scheduler(opt_lip_di, opt.non_decay, opt.decay, lr_policy=self.opt.scheduler)
        return [opt_g, opt_di, opt_di_2, opt_lip_di], [
            self.net_g_scheduler,
            self.net_dI_scheduler,
            self.net_dI_2_scheduler,
...
if __name__ == "__main__":
    ...
    from lipsync.dinet.ds_adv import load_concat_data
